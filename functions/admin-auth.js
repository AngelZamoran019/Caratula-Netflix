const COOKIE_NAME = "caratula_netflix_session";
...
async function createSession(env) {
  ...
  const signature = await createSignature(
    payload,
    getEnv(env, "ADMIN_SESSION_SECRET")
  );
...
async function verifySession(token, env) {
  ...
  const secret = getEnv(env, "ADMIN_SESSION_SECRET");
...
export async function onRequest(context) {
  ...
  try {
    ...
    const expectedUsername = getEnv(
      env,
      "ADMIN_USERNAME"
    );
    const expectedPassword = getEnv(
      env,
      "ADMIN_PASSWORD"
    );
